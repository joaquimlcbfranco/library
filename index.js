...
function displayBook(id) {
	...
	if (myLibrary[findLibraryIndex(id)].read) {
		...
		cardButtons.innerHTML = eyeSvg;
		cardImage.classList.add('notread');
	}
...
form.addEventListener('submit', (e) => {
	...
	const re2 = /\.{0,1}\/{1}(.*\/)*.*\.{1}/;
	if (re1.test(formLink) || re2.test(formLink) || !formLink) {
...
dialogForm.addEventListener('submit', (e) => {
	e.preventDefault();
	const re3 = /(http){0,1}s{0,1}(:\/\/){0,1}(www.){0,1}.*\.{1}.*/;
	...
	if (re3.test(dialogLink.value) || !dialogLink.value) {
	...
	const cardLink = document.querySelector(`.card-image[data-id="${currentCardIndex}"`);

	cardHeader.textContent = dialogTitle.value;
	cardAuthor.textContent = dialogAuthor.value;
	cardPages.textContent = +dialogPages.value;
	if (re3.test(dialogLink.value)) {
		cardLink.style.backgroundImage = `url(${dialogLink.value})`;
	}
	else if (!dialogLink.value) {
		cardLink.style.background = 'linear-gradient(to right bottom,rgb(58, 77, 57) 0%,rgb(58, 77, 57) 50%,rgb(79, 111, 82) 50%,rgb(79, 111, 82) 100%)';
	}
...
function readStatus(id) {
	...
	currentParent = document.querySelector(`.card-image[data-id="${id}"]`)
	if (myLibrary[findLibraryIndex(id)].read) {
		...
		currentParent.classList.add('notread');
	}
		...
		currentParent.classList.remove('notread');
	}
}
